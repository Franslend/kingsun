...
toggleBtn.addEventListener( 'click',  (event) => {
	...
	if(sideBarIsOpen){			
		...
		document.getElementsByClassName('dashboard_menu_lists')[0].style.textAlign = 'right';
		sideBarIsOpen = true;
	}
});
...
function showHideSubMenu(subMenu,mainMenuIcon){
	...
	if(subMenu != null){
		if(subMenu.style.display === 'block') {
			...
			mainMenuIcon.classList.add('fa-angle-right');				
		} else {
			subMenu.style.display = 'block';
			mainMenuIcon.classList.remove('fa-angle-right');
...
mainNav.style.background = '#337Ab7';
